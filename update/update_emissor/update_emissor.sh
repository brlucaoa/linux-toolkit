...
echo "${INFO}Iniciando atualizacao do $EMISSOR_NAME em producao...${DFLT}"

echo "${INFO}Parando container...${DFLT}"
docker stop $EMISSOR_NAME 2>/dev/null

echo "${INFO}Removendo container...${DFLT}"
docker container rm $EMISSOR_NAME 2>/dev/null

echo "${INFO}Removendo imagem...${DFLT}"
docker rmi $REGISTRY/$EMISSOR:latest 2>/dev/null
...
echo "V2=[$V2]"
echo "V3=[$V3]"
echo "IMAGE=[$REGISTRY/$EMISSOR:latest]"
