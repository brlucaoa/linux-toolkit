...
echo "${INFO}Iniciando atualizacao do $WSINT_NAME em producao...${DFLT}"

echo "${INFO}Parando container...${DFLT}"
docker stop $WSINT_NAME 2>/dev/null

echo "${INFO}Removendo container...${DFLT}"
docker container rm $WSINT_NAME 2>/dev/null

echo "${INFO}Removendo imagem...${DFLT}"
docker rmi $REGISTRY/$WSINT:latest 2>/dev/null
...
echo "V2=[$V2]"
echo "V3=[$V3]"
echo "IMAGE=[$REGISTRY/$WSINT:latest]"
